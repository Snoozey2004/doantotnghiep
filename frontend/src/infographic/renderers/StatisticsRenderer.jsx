...
export default function StatisticsRenderer({ data }) {
  ...
  return (
    <div className={visibilityClass} style={{
      ...
      <section style={{ padding: '30px 20px', backgroundColor: data?.backgroundColor || '#f4ecd8' }}>
        <div className="container" style={{ maxWidth: '1000px', margin: '0 auto' }}>
          {data?.title && (
            <h2 style={{ textAlign: 'center', fontSize: '2rem', color: textColor || '#8b4513', marginBottom: '40px' }}>
              {data.title}
            </h2>
          )}
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '30px', justifyContent: 'center' }}>
            {stats.map((stat, idx) => (
              ...
              >
                <div style={{ fontSize: '3rem', fontWeight: '800', color: textColor || '#d2691e', marginBottom: '10px' }}>
                  {stat.value}
                </div>
                <div style={{ fontSize: '1.1rem', color: textColor || '#666', fontWeight: '500' }}>
                  {stat.label}
                </div>
              </motion.div>
            ))}
