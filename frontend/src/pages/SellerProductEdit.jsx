...
export default function SellerProductEdit() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (id) {
        const offer = await productOfferApi.getOffer(id);
        if (offer) {
          setForm({
            ...
            isOpen: offer.isOpen !== undefined ? offer.isOpen : true
          });
          if (offer.businessHours) {
            const parts = offer.businessHours.split(" - ");
            if (parts.length === 2) {
              setOpenTime(parts[0].trim());
              setCloseTime(parts[1].trim());
            }
          }
          const prod = sortedData.find(p => p.id === offer.productId);
          if (prod) setProductSearchTerm(prod.name);
        }
      }
    } catch (err) {
      console.error(err);
  ...
  const handleSubmit = async (event) => {
    ...
    setLoading(true);
    try {
      await productOfferApi.updateOffer(id, payload);
      navigate("/seller");
    } catch (err) {
      console.error(err);
  ...
  return (
    <SellerLayout>
      <div className="admin-header">
        <div>
          <h1>Chỉnh Sửa Thông Tin Bán Hàng</h1>
          <p>Cập nhật giá bán, số lượng hoặc thông tin cửa hàng của bạn.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card" style={{ padding: "30px" }}>
        ...
        <div style={{ marginTop: "30px", display: "flex", gap: "15px" }}>
          <button disabled={loading} type="submit" style={{ padding: "14px 24px", background: "#e11d48", color: "white", borderRadius: "8px", border: "none", fontSize: "1.1rem", fontWeight: "bold", cursor: loading ? "not-allowed" : "pointer" }}>
            {loading ? "Đang lưu..." : "Cập Nhật Cửa Hàng"}
          </button>
